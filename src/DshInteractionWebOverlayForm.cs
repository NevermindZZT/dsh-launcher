...
internal sealed class DshInteractionWebOverlayForm : Form
{
    ...
    private void ApplyWindowPalette(ThemeHelper.Palette palette)
    {
        ...
        _web.DefaultBackgroundColor = palette.Surface;
        if (IsHandleCreated)
        {
    ...
    private void ApplyContentSize(int contentHeight)
    {
        if (contentHeight <= 0 || IsDisposed) return;
        if (InvokeRequired)
        {
            BeginInvoke(() => ApplyContentSize(contentHeight));
            return;
        }
        var chromeHeight = Math.Max(0, Height - ClientSize.Height);
        var minClientHeight = Math.Max(160, MinimumSize.Height - chromeHeight);
        var maxClientHeight = Math.Max(minClientHeight, MaximumSize.Height - chromeHeight);
        var targetClientHeight = Math.Clamp(contentHeight, minClientHeight, maxClientHeight);
        if (Math.Abs(ClientSize.Height - targetClientHeight) < 2) return;
    ...
    private void HandleWebMessage(string raw)
    {
        try
        {
            ...
            if (TextOf(root, "type") == "interaction-size")
            {
                if (root.TryGetProperty("height", out var height) && height.TryGetInt32(out var contentHeight))
...
setTimeout(reportSize,0);setTimeout(reportSize,80);setTimeout(reportSize,250);app.addEventListener('click',e=>{let a=e.target.closest('[data-a]')?.dataset.a;if(!a||busy)return;if(a==='cancel'||a==='allow'||a==='reject'){setBusy(true);send({type:'interaction',action:a})}});
</script></body></html>
""";
}
