...
function save(){let draft={attachPort:+$('#port').value||0,workingDirectory:$('#wd').value,closeExits:document.querySelector('input[name=close]:checked').value==='exit',autoStart:$('#auto').checked,openLinksInWebView:$('#links').checked,handleAgentQuestions:$('#agentQuestions').checked,manager:{enabled:$('#managerEnabled').checked,serverUrl:$('#managerUrl').value,agentName:$('#managerName').value,pairingCode:$('#managerPairing').value}};send('save',draft)}
