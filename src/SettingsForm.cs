...
public sealed class SettingsForm : ThemedForm
{
    ...
    private readonly ThemedCheckBox _chkHandleAgentQuestions = new() { Text = "同时显示 Launcher 的 Agent 提问和权限浮窗" };
    ...
    public SettingsForm(AppSettings settings)
    {
        ...
        row++;
        panel.Controls.Add(MkLabel("Agent 交互浮窗"), 0, row);
        ...
        interactionWrap.Controls.Add(MkLabel("dsh 原生 Web UI 始终显示；关闭后仅关闭 Launcher 浮窗和通知。", muted: true));
